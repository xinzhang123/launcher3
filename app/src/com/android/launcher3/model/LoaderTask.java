...
//            mResults.bindAllApps();//oh21 加载AllAppsContainerView。看后续时候有更新icon的bug
    ...
    private void loadWorkspace() {
        ...
        synchronized (mBgDataModel) {
            ...
            mBgDataModel.workspaceScreens.addAll(LauncherModel.loadWorkspaceScreensDb(context)); //oh21 data这里加载workspace数据
    ...
    private boolean isHideApplication(Context context, ComponentName cn){
        if(null == mAppFilter){
            mAppFilter = AppFilter.newInstance(context);
        }
        return !mAppFilter.shouldShowApp(cn);
    }
